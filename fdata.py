...
#		self._hist = fhist()
	...
	def paraview(self,exe = 'paraview',filename = 'temp.vtk',contour = None):
		...
		if WINDOWS:
			...
			p = Popen(exe+' --data='+wd+slash+self._vtk.path.filename+' --script=pyfehm_paraview_startup.py',shell=True)		
	def visit(self,exe = 'visit',filename = 'temp.vtk',contour = None):
		...
		if WINDOWS:
		    p = Popen(exe+' -o '+wd+slash+self._vtk.path.filename)		
		else:
		    p = Popen(exe+' -o '+wd+slash+self._vtk.path.filename,shell=True)		
	def _summary(self):		
		L = 62
		print ''
		print ' ****---------------------------------------------------------****'
