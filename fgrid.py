...
class gmake(object): 				#Gridder instruction object.
	""" Gridder make file. 
	
	"""	
	def __init__(self,meshname='',dimension=3,xmin=0.,xmax=1.,ymin=0.,ymax=0.,zmin=0.,zmax=0.,prop=1.,
				xprop=None,yprop=None,zprop=None,div=1,xdiv=None,ydiv=None,zdiv=None,x=[],y=[],z=[],
				exec_path='gridder',filename='gridder_input.dat'):
		...
		self._y=y
		self._z=z
		if isinstance(prop,int) or isinstance(prop,float): prop = [prop,]
		if isinstance(xprop,int) or isinstance(xprop,float): xprop = [xprop,]
		if isinstance(yprop,int) or isinstance(yprop,float): yprop = [yprop,]
		if isinstance(zprop,int) or isinstance(zprop,float): zprop = [zprop,]
		self._prop = prop
		self._xprop = xprop
		self._yprop = yprop
		self._zprop = zprop
		if isinstance(div,int) or isinstance(div,float): div = [div,]
		if isinstance(xdiv,int) or isinstance(xdiv,float): xdiv = [xdiv,]
		if isinstance(ydiv,int) or isinstance(ydiv,float): ydiv = [ydiv,]
		if isinstance(zdiv,int) or isinstance(zdiv,float): zdiv = [zdiv,]
		self._div = div
		self._xdiv = xdiv
		self._ydiv = ydiv
		self._zdiv = zdiv	
		...
		self._dimension = dimension
	def __repr__(self): return 'gridder constructor for '+str(self.filename)	
	def write(self,filename=''): 					#Write out gridder instructions.
		'''Write gridder instructions to output file.
		
		:param filename: Name of file to write gridder instructions to.
		:type filename: str		
		...
		xflag = len(self.x)>0; yflag = len(self.y)>0; zflag = len(self.z)>0
		if (xflag and yflag and self._dimension==2) or(xflag and yflag and zflag and self._dimension==3):
			outfile.write(str(self._dimension)+'\n')

			self.x = np.sort(self.x); self.y = np.sort(self.y); 
			if zflag: self.z = np.sort(self.z)
			self.xmin = self.x[0]; self.ymin = self.y[0]; 
			if zflag: self.zmin = self.z[0]
			self.xmax = self.x[-1]; self.ymax = self.y[-1]; 
			if zflag: self.zmax = self.z[-1]
			
			self.xprop = list(np.diff(self.x)/float(self.xmax-self.xmin))
			self.yprop = list(np.diff(self.y)/float(self.ymax-self.ymin))
			if zflag: self.zprop = list(np.diff(self.z)/float(self.zmax-self.zmin))
			
			self.xdiv = [1 for i in self.xprop]
			self.ydiv = [1 for i in self.yprop]
			if zflag: self.zdiv = [1 for i in self.zprop]
			
			if self.dimension == 2: props = [self.xprop,self.yprop]
			else: props = [self.xprop,self.yprop,self.zprop]

			if self.dimension == 2: divs = [self.xdiv,self.ydiv]
			else: divs = [self.xdiv,self.ydiv,self.zdiv]

			if self.dimension == 2: mins = [self.xmin,self.ymin]
			else: mins = [self.xmin,self.ymin,self.zmin]

			if self.dimension == 2: maxs = [self.xmax,self.ymax]
			else: maxs = [self.xmax,self.ymax,self.zmax]
			
			print props
			print divs
			print mins
			print maxs
			for prop,div,min,max in zip(props,divs,mins,maxs):		
				# extract and normalise proprtional divisions
				if prop == None: prop = self.prop
				prop = np.array([float(p) for p in prop])
				prop = np.cumsum(prop/np.sum(prop)) 	# normalise
				outfile.write(str(len(prop))+'\n')
				min,max = np.min([min,max]), np.max([min,max])
				outfile.write(str(min)+'\n')
				if div == None: div = self.div
				for di,divi in zip(div,prop):
					outfile.write(str(min+divi*(max-min))+'\n');
					outfile.write(str(di)+'\n');
					outfile.write('1\n')
			outfile.write('4\n')
			outfile.close()
		else:
	...
	def run(self,meshname='',exec_path=''):						#Run gridder.
		'''Write out gridder instructions and execute to generate a grid.
		
		:param meshname: Name of FEHM grid file to construct.
		:type meshname: str
		:param exec_path: Path to gridder executable, e.g., 'c:\\python\\pyfehm\\gridder.exe'.
		:type exec_path: str
		'''
		if meshname: self.meshname = meshname
		if exec_path: self.exec_path = exec_path
		# check if file exists
		if WINDOWS:
			if self.exec_path.endswith('.exe'): isexec = os.path.isfile(self.exec_path)
			else: isexec = os.path.isfile(self.exec_path+'.exe')
			if not isexec: print 'No executable of that name found...'; return		
		new_dir = False
		exec_name = self.exec_path
		if slash in self.exec_path: # path to executable provided
			original_dir = os.getcwd()
			new_dir = ''
			for dir in self.exec_path.split(slash)[:-1]: new_dir += dir + slash
			os.chdir(new_dir)
			new_dir = True			
			exec_name = self.exec_path.split(slash)[-1]
		self.write()
		
		os.system(self.exec_path + ' < '+self.filename+ ' > nul')
		if self.meshname:
			os.system(copyStr+' grid.inp '+self.meshname + ' > nul ')
			os.system(delStr+' grid.inp')
		os.system(delStr+' '+self.filename)
		if new_dir:
			if self.meshname:
				os.system(copyStr+' '+self.meshname+' '+original_dir+slash+self.meshname+' > nul')
				os.system(delStr+' '+self.meshname)
			else:
				os.system(copyStr+' grid.inp '+original_dir+'grid.inp > nul')
				os.system(delStr+' grid.inp')
			os.chdir(original_dir)
	def _get_info(self): 
		prntStr = 'Grid file to be written to '
		if self._meshname: prnStr += self._meshname +'.\n'
		else: prntStr += 'grid.inp.\n'
		prntStr += 'Grid will contain:\n'
		prntStr += '    x-divisions:'+str(len(self.x)-1)+'\n'
		prntStr += '    y-divisions:'+str(len(self.y)-1)+'\n'
		prntStr += '    z-divisions:'+str(len(self.z)-1)+'\n'
		prntStr += '    elements:'+str((len(self.x)-1)*(len(self.y)-1)*(len(self.z)-1))+'\n'
		prntStr += '    nodes:'+str((len(self.x))*(len(self.y))*(len(self.z)))+'\n'
		
		print prntStr
	what = property(_get_info) 		#: Print out information about the object.
	def _get_filename(self): return self._filename
	def _set_filename(self,value): self._filename = value
	filename = property(_get_filename,_set_filename) 	#: (*str*) Name of file to write gridder instructions.	
	def _get_meshname(self): return self._meshname
	def _set_meshname(self,value): self._meshname = value
	meshname = property(_get_meshname,_set_meshname) 	#: (*str*) Name of grid file to be created.	
	def _get_xmin(self): return self._xmin
	def _set_xmin(self,value): self._xmin=value
	xmin = property(_get_xmin,_set_xmin)#: (*fl64*) Minimum x-coordinate of grid.
	def _get_xmax(self): return self._xmax
	def _set_xmax(self,value): self._xmax=value
	xmax = property(_get_xmax,_set_xmax)#: (*fl64*) Maximum x-coordinate of grid.
	def _get_ymin(self): return self._ymin
	def _set_ymin(self,value): self._ymin=value
	ymin = property(_get_ymin,_set_ymin)#: (*fl64*) Minimum y-coordinate of grid.
	def _get_ymax(self): return self._ymax
	def _set_ymax(self,value): self._ymax=value
	ymax = property(_get_ymax,_set_ymax)#: (*fl64*) Maximum y-coordinate of grid.
	def _get_zmin(self): return self._zmin
	def _set_zmin(self,value): self._zmin=value
	zmin = property(_get_zmin,_set_zmin)#: (*fl64*) Minimum z-coordinate of grid.
	def _get_zmax(self): return self._zmax
	def _set_zmax(self,value): self._zmax=value
	zmax = property(_get_zmax,_set_zmax)#: (*fl64*) Maximum z-coordinate of grid.
	def _get_prop(self): return self._prop
	def _set_prop(self,value): self._prop=value
	prop = property(_get_prop,_set_prop)#: (*lst[fl64]*) Default proportions for division of grid axes into regions.
	def _get_xprop(self): return self._xprop
	def _set_xprop(self,value): self._xprop=value
	xprop = property(_get_xprop,_set_xprop)#: (*lst[fl64]*) Proportions for division of x-axis into regions.
	def _get_yprop(self): return self._yprop
	def _set_yprop(self,value): self._yprop=value
	yprop = property(_get_yprop,_set_yprop)#: (*lst[fl64]*) Proportions for division of y-axis into regions
	def _get_zprop(self): return self._zprop
	def _set_zprop(self,value): self._zprop=value
	zprop = property(_get_zprop,_set_zprop)#: (*lst[fl64]*) Proportions for division of z-axis into regions
	def _get_div(self): return self._div
	def _set_div(self,value): self._div=value
	div = property(_get_div,_set_div)#: (*lst[fl64]*) Default element divisions to assign each region.
	def _get_xdiv(self): return self._xdiv
	def _set_xdiv(self,value): self._xdiv=value
	xdiv = property(_get_xdiv,_set_xdiv)#: (*lst[fl64]*) Element divisions to assign each region of x-axis.
	def _get_ydiv(self): return self._ydiv
	def _set_ydiv(self,value): self._ydiv=value
	ydiv = property(_get_ydiv,_set_ydiv)#: (*lst[fl64]*) Element divisions to assign each region of y-axis.
	def _get_zdiv(self): return self._zdiv
	def _set_zdiv(self,value): self._zdiv=value
	zdiv = property(_get_zdiv,_set_zdiv)#: (*lst[fl64]*) Element divisions to assign each region of z-axis.
	def _get_executable(self): return self._executable
	def _set_executable(self,value): self._executable=value
	exec_path = property(_get_executable,_set_executable) 	# (*str*) Name of gridder executable. May include path specification, e.g., c:\\users\\gridder
	def _get_dimension(self): return self._dimension
	def _set_dimension(self,value): self._dimension=value
	dimension = property(_get_dimension,_set_dimension)
	def _get_x(self): return self._x
	def _set_x(self,value): self._x = value
	x = property(_get_x, _set_x) #: (*lst[fl64]*) List of coordinates for the x-axis.
	def _get_y(self): return self._y
	def _set_y(self,value): self._y = value
	y = property(_get_y, _set_y) #: (*lst[fl64]*) List of coordinates for the y-axis.
	def _get_z(self): return self._z
	def _set_z(self,value): self._z = value
	z = property(_get_z, _set_z) #: (*lst[fl64]*) List of coordinates for the z-axis.
